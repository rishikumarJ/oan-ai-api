...
def dynamic_system_prompt(ctx: RunContext[FarmerContext]) -> str:
    ...
    return get_prompt(lang, context={'today_date': get_today_date_str(lang)})
...
def generation_system_prompt(ctx: RunContext[FarmerContext]) -> str:
    ...
    try:
        ...
        return get_prompt(f"generation_{lang}", context={'today_date': get_today_date_str(lang)})
    except Exception:
        # Fallback to standard prompt with explicit override
        base_prompt = get_prompt(lang, context={'today_date': get_today_date_str(lang)})
